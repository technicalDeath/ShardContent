...
3. Replace murder adjudication with the approved automatic model.
   ...
   - Current implementation status: `MurderAdjudicationService` contains the tested policy, account-tag ledger, persisted encounter snapshots, and feature-gated `PlayerDeathEvent` hook behind `featureFlags.automaticMurderAdjudication`; the ModernUO fork exposes the narrow legacy-reporting disable, legacy-red suppression, and additional-red-status hooks. Migration audit and public enablement remain gated work; the live profile intentionally leaves this flag off.
