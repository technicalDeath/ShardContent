...
1. Implement a central player-hostility decision service.
   ...
   - Current implementation status: `PvpIntentService` is the single stock-boundary decision point and resolves controlled-creature hostility to the player master before applying the same consent/retaliation policy; Alpha 2 flags are dependency-validated but remain disabled in the current Alpha 1 profile until region and migration gates are ready.
...
5. Implement the Knocked Out state and safe-world resolution path.
   ...
   - Current implementation status: the feature-gated `KnockedOutService`, narrow ModernUO lethal-damage/damageability hooks, and stock Stealing no-skill boundary persist a 90-second ordinary-blue state, clear combat/targets, reject further damage, retain a completed-encounter record, permit only the recorded criminal/red engagement holder to loot backpack items, and expose encounter-authorized `[Execute` with automatic murder adjudication; Hot/Cool resolution and live enablement remain gated work.
