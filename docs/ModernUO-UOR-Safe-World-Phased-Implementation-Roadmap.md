...
4. Preserve crime while adding narrowly targeted anti-harassment protections.
   ...
   - Current implementation status: the feature-gated `BackpackWard` and stock Stealing/Corpse pre/post hooks implement deterministic physical-Ward selection, per-thief-account 25%/50%/100% escalation, post-resolution consumption, persistent 120-second victim protection, first-transfer/ten-minute-repeat protection for unlawful non-Hot monster corpses, character-creation starter issuance with durable account binding, and data-driven bank/Cool polygon checks at the theft boundary. The checked-in geometry lists are empty pending approved map coordinates; live enablement remains gated work.
