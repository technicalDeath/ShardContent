...
3. Replace murder adjudication with the approved automatic model.
   ...
   - Current implementation status: `MurderAdjudicationService` contains the tested policy, account-tag ledger, persisted encounter snapshots, and feature-gated `PlayerDeathEvent` hook behind `featureFlags.automaticMurderAdjudication`; the ModernUO fork exposes the narrow legacy-reporting disable, legacy-red suppression, and additional-red-status hooks. A read-only production `[MurderMigrationAudit` on 2026-09-24 scanned one player with zero legacy-threshold, custom-ledger, active-custom-red, or overlapping rows and performed no mutation. Migration treatment for future non-neutral rows and public enablement remain gated work; the live profile intentionally leaves this flag off.
